...
void test_fb_search_node(fb_tree *tree, fb_block_h *block, int key)
{
	...
	char status;
	size_t key_pos = 0;
	size_t pos = _fb_search_node(tree, block, node_pos, key, &key_pos, &status);
	printf("_fb_search_node: seek %i | status %i | pos %zu | key_pos %zu\n", 
...
void test_fb_search_block(fb_tree *tree, fb_block_h *block, int key)
{
	...
	printf("_fb_search_block: seek %i | status %i | content %zu | node_pos %zu | leaf_pos %zu\n",
...
void test_fb_get(fb_tree *tree, int key)
{
	...
	printf("_fb_get: seek %i | found %i | status %i | content %zu | block_pos %zu | node_pos %zu | leaf_pos %zu\n",
